...
export async function redirectToSupporterCheckout(userId, userEmail) {
  try {
    const { data: { session } } = await supabase.auth.getSession()
    const token = session?.access_token
    ...
    const response = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/supporter-checkout`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (error || !url) {
      console.error('Supporter checkout session error:', error ?? 'no URL returned')
      return
    }
    ...
    console.error('Failed to create supporter checkout session:', err)
  }
}
